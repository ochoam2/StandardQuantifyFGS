...
%% read MP4 ==========================================
% (These reads the files as acquired through the varying distances)
maindir = cd;
datadir = 'OnLume-2023-01-24-01';
cd(datadir);
b1 = imread('15cm.tif');
b2 = imread('14cm.tif');
b3 = imread('13cm.tif');
...
ang = 0;
for k = 1:n
img = eval(['b' num2str(k)]);
img = sum(img,3);
norm = imrotate(img,ang,'crop');
norm = norm./max(norm(:));
norm(norm<0.2*max(max(norm))) = 0;
allimgs{1,k} = norm;
figure(1); subplot(3,8,k); imagesc(norm); colormap hot;  title(num2str(k)); caxis([0 0.8]);
end
...
save('depthoffieldh3','ctfsjoin','ctfsjoinmean');
